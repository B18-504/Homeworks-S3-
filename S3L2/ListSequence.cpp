...
void ListSequence<T>::Append(T &value)
{
	...
	if(Sequence<T>::len)
	{
		last->next = new Node(&value, 0, last);
		last = last->next;
	}
	else
	{
...
void ListSequence<T>::Prepend(T &value)
{
	...
	if(Sequence<T>::len)
	{
		first->prev = new Node(&value, first, 0);
		first = first->prev;
	}
	else
	{
...
void ListSequence<T>::Insert(T &value, int index)
{
	...
	else
	{
		...
		tmp->next = new Node(&value, tmp->next, tmp);
...
template <typename T>
void ListSequence<T>::ForceLength(int length)
{
	if(length < 0)
	{
		throw NLE();
	...
	//it makes sence to pre-allocate a memory chunk for a group of future nodes
}
